...
addEithers (Right b) (Left a) = Left a
addEithers (Left a) (Left b) = Left a
addEithers (Right c) (Right d) = Right (c + d)
...
data Person = OwnPerson Int String
  deriving Show
...
fred = OwnPerson 90 "Fred"
-- getName returns the name of the person
getName :: Person -> String
--getName p = undefined
getName (OwnPerson _ name) = name
-- getAge returns the age of the person
getAge :: Person -> Int
--getAge p = undefined
getAge (OwnPerson age _ ) = age
-- setName takes a person and returns a new person with the name changed
setName :: String -> Person -> Person
--setName name p = undefined
setName name (OwnPerson age _name) = OwnPerson age name 
-- setAge does likewise for age
setAge :: Int -> Person -> Person
--setAge age p = undefined
setAge age (OwnPerson _age name) = OwnPerson age name 
------------------------------------------------------------------------------
...
data UpDown = OwnUp Int | OwnDown Int
...
tick :: UpDown -> UpDown
--tick ud = undefined
tick (OwnUp a) = OwnUp (a + 1)
tick (OwnDown a) = OwnDown (a - 1)

-- toggle changes an increasing counter into a decreasing counter and
-- vice versa
toggle :: UpDown -> UpDown
--toggle ud = undefined
toggle (OwnUp a) = OwnDown a
toggle (OwnDown a) = OwnUp a
...
treeSize Leaf = 0
treeSize (Node _ b c) = treeSize b + treeSize c + 1
...
leftest (Node x Leaf _) = Just x  -- (1+2)
leftest (Node x c@(Node _ _ _) _) = leftest c  -- (3+4)
...
--mapTree x t = undefined
mapTree _ Leaf = Leaf 
mapTree f (Node x v o) = Node (f x)(mapTree f v) (mapTree f o)
...
rgb (Mix a b) = (map saturate) (zipWith (+) (rgb a) (rgb b)) 
 where saturate x = min x 1
rgb (Darken c d) = map (*scale) (rgb d)
  where scale = 1-c
