...
measure s | length s > 0 = length s |otherwise = -1-- > 0 = length s | otherwise length s = -1 -- -> int-- = -1-- length s --| null s
...
remove 0 (x:xs) = xs
remove i (x:xs) | i >= 0 = x : (remove (i-1) xs)
...
substring i n s = take ((n + i) - i) (drop i s)
...
mymax measure a b| measure a < measure b = b |otherwise = a
...
allPowers n = 1 : map (* n) (allPowers n)


powers :: Int -> Int -> [Int]
--powers n max = undefined

powers n max = takeWhile(<= max) (allPowers n)
...
sorted [] = True
sorted [x] = True
sorted (x:y:xs) = x <= y && sorted (y:xs)-- else false
